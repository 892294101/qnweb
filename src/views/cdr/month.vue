  ...
  <el-card body-style="padding:15px; margin-top:5px; ">
    ...
    <el-form :inline="true" :model="queryParams" ref="queryFormRef" label-width="70px">
      <el-form-item label="日期" prop="StartTime">
        <el-date-picker v-model="queryParams.StartTime" type="monthrange" unlink-panels range-separator="-" start-placeholder="开始时间" end-placeholder="结束时间" @keydown.enter.capture="queryCDR(queryParams)"/>
      </el-form-item>
    ...
    <el-card body-style="padding:15px; margin-top:5px;">
      <el-form :inline="true">
        <el-table size="small" v-loading="Loading" border stripe :data="queryList" style="width: 100%;" :header-cell-style="headerCellStyle" @cell-click="CopyText">
          ...
          <el-table-column label="接通率%" prop="ConnectedRate" min-width="80" width="auto" :sortable="true" :sort-orders="sortOrders"/>
        </el-table>
...
const getCDRMonthList = (async (e) => {
  Loading.value = true
  const {data: res} = await api.getCDRMonthList(e)
  if (res.code !== 200) {
